        ...
        <form method="POST" action="{{ route('kilometrajes.store') }}" id="kilometraje-form">
            ...
            <div class="mb-6">
                ...
                <div class="relative">
                    ...
                    <input type="hidden" id="obra_id" name="obra_id" value="{{ old('obra_id') }}">
                ...
                <p class="text-gray-600 text-sm mt-1">
                    ...
                    Selecciona la obra si el kilometraje está relacionado con una actividad específica
                </p>
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea class="w-full p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('observaciones') border-red-500 @enderror" 
                          ...
                          rows="4"
                          maxlength="500"
                          placeholder="Ej: Recorrido a obra zona norte, abastecimiento de combustible, mantenimiento preventivo...">{{ old('observaciones') }}</textarea>
                @error('observaciones')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
                <p class="text-gray-600 text-sm mt-1">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Si hay una obra pre-seleccionada (por old values), actualizar campo de búsqueda
    const obraIdOld = document.getElementById('obra_id').value;
    if (obraIdOld) {
        const obraOption = document.querySelector(`[data-id="${obraIdOld}"]`);
        if (obraOption) {
            document.getElementById('obra_search').value = obraOption.dataset.text;
        }
    }
    
    // Configurar event listeners para los dropdowns
