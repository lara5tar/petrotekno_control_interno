...
@section('title', 'Puestos de Personal')

@section('header', 'Gestión de Puestos de Personal')
...
@section('content')
    ...
    <x-breadcrumb :items="[
        ...
        ['label' => 'Puestos de Personal']
    ]" />

    <!-- Encabezado con botones de agregar -->
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Puestos de Personal</h2>
    ...
    <div class="bg-white p-4 rounded-lg shadow-md mb-6">
        <form method="GET" action="{{ route('categorias-personal.index') }}" id="filtrosForm">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div class="flex-1">
                    <label for="search" class="block text-sm font-medium text-gray-700 mb-1">Buscar puesto</label>
                    <div class="relative">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Nombre del Puesto</th>
                        ...
                        <tr>
                            <td colspan="5" class="px-6 py-8 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <h3 class="mt-2 text-sm font-medium text-gray-900">No hay puestos</h3>
                                    <p class="mt-1 text-sm text-gray-500">
                                        @if(request()->filled('search'))
                                            No se encontraron puestos con los criterios especificados.
                                        @else
                                            Aún no hay puestos de personal registrados en el sistema.
                                        @endif
                                    </p>
                                    @if(request()->filled('search'))
                                        <div class="mt-4">
                    ...
                    <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                        <div>
                            <p class="text-sm text-gray-700">
                                Mostrando <span class="font-medium">{{ $categorias->firstItem() }}</span> a <span class="font-medium">{{ $categorias->lastItem() }}</span> de <span class="font-medium">{{ $categorias->total() }}</span> puestos
                            </p>
                        </div>
                        <div>
        ...
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div class="mt-3">
                ...
                <div class="flex items-center justify-between pb-3">
                    <h3 class="text-lg font-medium text-gray-900">Nuevo Puesto de Personal</h3>
...
// Funciones para el modal de crear puesto
...
function submitCreateCategory(event) {
    ...
    // Usar la URL tal como está configurada
    ...
    .then(data => {
        if (data.success) {
            ...
            showSuccessMessage('Puesto creado exitosamente');
            
            // Recargar la página para mostrar el nuevo puesto
            setTimeout(() => {
            ...
            if (data.errors && data.errors.nombre_categoria) {
                ...
                errorDiv.textContent = data.message || 'Error al crear el puesto';
            }
    ...
    if (deleteForm) {
        deleteForm.addEventListener('submit', function(e) {
            ...
            .then(data => {
                if (data.success) {
                    // Mostrar mensaje de éxito
                    showSuccessMessage(data.message || 'Puesto eliminado exitosamente');
